...
@Component({
    ...
    template:`<span class='telnumber'>{{distributionRole?.name}}&nbsp;{{distributionRole?.mobile}}&nbsp;&nbsp;</span>
              <span class='phone'><ion-icon ios="ios-call-outline"></ion-icon></span>
            `
})
export class Distribution {
    ...
    ngOnInit() {
        let vm = this
        let url = '/api/account/employees/' + vm.r
        axios
            .get(url)
            .then(function (res) {
                console.log(res)
                vm.distributionRole = res.data
            })
    }
    onClick() {
        this.notify.emit('Click from nested content');
